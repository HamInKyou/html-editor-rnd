...
export function Editor() {
  ...
  return (
    <Slate editor={editor} initialValue={initialValue}>
      <Editable renderElement={renderElement} renderLeaf={renderLeaf} onKeyDown={handleKeyDown} />
    </Slate>
  );
}

function Element(props: RenderElementProps) {
  const strategies: ElementRenderStrategy[] = [new CodeElementRenderStrategy(), new ParagraphElementRenderStrategy()];

  const strategy = strategies.find((s) => s.type === props.element.type) ?? new ParagraphElementRenderStrategy();
